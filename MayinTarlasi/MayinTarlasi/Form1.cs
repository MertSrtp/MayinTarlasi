...
namespace MayinTarlasi
{
    public partial class Form1 : Form
    {
        ...
        private void InitializeGame()
        {
            ...
            // Daha küçük buton boyutu kullanarak grid'i sıkılaştıralım
            int buttonSize = 30; // Sabit küçük buton boyutu
            int padding = 2;     // Butonlar arası boşluk
            
            // Grid'in toplam boyutunu hesapla
            int gridWidth = (buttonSize + padding) * GridSize - padding;
            int gridHeight = (buttonSize + padding) * GridSize - padding;
            
            // Grid'i form içinde ortala
            int startX = (this.ClientSize.Width - gridWidth) / 2;
            int startY = resetButton.Bottom + 10;
            
            // Form boyutunu ayarla - biraz daha kompakt
            this.ClientSize = new Size(
                Math.Max(300, gridWidth + 40), // Minimum 300px genişlik
            ...
            for (int i = 0; i < GridSize; i++)
            {
                for (int j = 0; j < GridSize; j++)
                {
                    buttons[i, j] = new Button
                    {
                        ...
                        Location = new Point(
                            startX + (j * (buttonSize + padding)), 
                            startY + (i * (buttonSize + padding))
                        ),
                        BackColor = Color.LightGray,
                        Tag = new Point(i, j),
                        Margin = new Padding(0),
                        Font = new Font("Arial", 8f) // Daha küçük font
                    };
